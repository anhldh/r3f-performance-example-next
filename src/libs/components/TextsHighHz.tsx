...
const TextHighHZ: FC<TextHighHZProps> = memo(
  ({
    ...
    useEvent("log", function updateR3FPerfText([log, gl]) {
      ...
      if (customData) {
        fpsRef.current.text = (
          Math.round(getPerf().customData * Math.pow(10, round)) /
          Math.pow(10, round)
        ).toFixed(round);
      }
      ...
      // --- FIX: Thêm Optional Chaining (?.) để tránh crash ---
      if (isShadersInfo) {
        info = gl?.info?.programs?.length || 0;
      } else if (metric === "matriceCount") {
        info = matriceCount.value;
      } else if (!isPerf) {
        // Kiểm tra an toàn trước khi truy cập render
        if (gl?.info?.render) {
          const infos: any = isMemory ? gl.info.memory : gl.info.render;
          info = infos ? infos[metric] : 0;
        }
      }
      // -----------------------------------------------------
      ...
      // Đảm bảo info là số trước khi toFixed
      const safeInfo = typeof info === "number" ? info : 0;
      fpsRef.current.text = (
        Math.round(safeInfo * Math.pow(10, round)) / Math.pow(10, round)
      ).toFixed(round);

      if (hasInstance) {
        // --- FIX: Kiểm tra an toàn cho instance ---
        ...
        if (metric === "matriceCount") {
          ...
          infoInstance = infosInstance ? infosInstance[metric] : 0;
        }

        if (infoInstance > 0) {
          fpsRef.current.fontSize = fontSize / 1.15;
          // Sử dụng safeInfo thay vì info cũ có thể undefined
          fpsInstanceRef.current.fontSize =
            safeInfo > 0 ? fontSize / 1.4 : fontSize;

          fpsRef.current.position.y = h / 2 - offsetY - fontSize / 1.9;
          fpsInstanceRef.current.text =
            " ± " +
            (
              Math.round(infoInstance * Math.pow(10, round)) /
              Math.pow(10, round)
            ).toFixed(round);
        } else {
    ...
    return (
      <Suspense fallback={null}>
        <Text
          ...
          fontSize={fontSize}
          position={[
            -w / 2 + offsetX + fontSize,
            h / 2 - offsetY - fontSize,
            0,
          ]}
          color={color}
...
TextHighHZ.displayName = "TextHighHZ";

export const TextsHighHZ: FC<PerfUIProps> = ({
  colorBlind,
  customData,
  minimal,
  matrixUpdate,
